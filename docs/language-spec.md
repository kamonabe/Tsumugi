...
現行実装には、同一scopeでの再宣言、未捕捉REPLエラー後の状態、error kind / messageの一部に両engine差がある。また、`path_join`の非文字列argument無視は両engine共通の仕様違反である。これらの挙動へ依存するコードは移植可能とみなさない。
...
- 関数値は同一の関数値とだけ等しい。`f == f`は`true`だが、同じ`fn`から作った別のクロージャは`false`
