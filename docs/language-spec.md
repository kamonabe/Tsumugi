...
現行実装では、parser・compiler・evaluatorの一部再帰経路と非循環import chainが関数コール深度制限の対象外で、host stack overflowに到達し得る。ステップ予算はloop反復と関数・callback呼び出しを中心に数え、構文解析、コンパイル、直線的な大量コード、文字列サイズ、重いbuiltin、I/O待ち、プロセス全体の総heap量は包括的に制限しない。filesystem制御にもsymlinkのTOCTOU制約がある。
...
ユーザー関数呼び出しのネスト深度を128フレームに制限し、通常の関数再帰をプロセスクラッシュ前にランタイムエラー化する。この上限は構文・AST・importの再帰には適用されず、AUD-027 / AUD-028は未解決である。
