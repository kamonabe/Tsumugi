...
本書をTsumugiの**現行実装から観測できる規範仕様**とし、デフォルトのツリーウォーク版と`--vm`版は同じ観測可能な挙動を満たすものとする。次期revisionについて設計確定済みだが未実装の決定は[次期意味論・実装決定](semantic-decisions.md)を正本とし、その受入基準とpaired testが完了するまで本書へ先行反映しない。実装差は言語機能ではなく非適合として扱い、詳細と修正状況は[`roadmap.md`](roadmap.md)の監査バックログで管理する。
...
- 値は任意の型
- 末尾カンマ許容: `{"a": 1, "b": 2,}`
- キーの順序は挿入順ではなくキーの昇順で保持される（`BTreeMap`）。比較はUnicodeコードポイント順のため、ASCII大文字がすべて小文字より先に来る（`"Z" < "a"`）。非ASCIIはコードポイントの並びに従う
...
同じスコープで同名を `let` で再宣言すると、**必ず新しい変数セルを作る**。そのスコープの名前解決は新しいセルへ切り替わるが、再宣言より前に作られたクロージャは旧セルを保持し、旧値を見続ける。関数定義（`fn`）も名前を現在のスコープへbindする宣言であり、同じ規則に従う。この挙動はトップレベル・関数本体・ブロックスコープ・REPL入力間のいずれでも一貫する。
...
再宣言と通常の再代入は区別される。再代入（`x = ...`）は解決済みの既存セルの中身を書き換えるため、そのセルを共有する全クロージャに反映される。再宣言（`let x = ...`）は新しいセルを割り当てるため、既存クロージャには影響しない。共有された状態を更新したい場合は `let` ではなく通常代入を使う。
...
すべての型の組み合わせで結果を返し、型エラーにはならない。

- 型が違う値は等しくない。`1 == "1"`、`1 == true`、`1 == null` はいずれも`false`
- 数値だけは例外で、IntとFloatを数値として比較する。`1 == 1.0`は`true`、`1 == 1.5`は`false`。比較はIntを`f64`へ丸めず数学的に厳密で、`2^53`を超える整数でも桁落ちしない。このため等価は対称かつ推移的で、`2^53`（`9007199254740992`）と`9007199254740993`は別の値として区別される（`9007199254740993 == 9007199254740992.0`は`false`。REV-003）
- 文字列・真偽値・nullは値で比較する
- リストと辞書は要素ごとに再帰的に比較する。`[1] == [1.0]`は数値比較が働くため`true`
- エラー値は`type` / `message` / `line`がすべて一致すれば等しい
- 関数値は同一の関数値とだけ等しい。関数式・関数定義を評価して値を生成するたびに新しい同一性が割り当てられ、代入・引数渡し・collection格納では同じ同一性を保つ。`f == f`は`true`だが、同じ`fn`式を複数回評価した別インスタンスは、捕捉の有無に関係なく`false`（AUD-048。デフォルト実行系・`--vm`とも同じ挙動）
- NaNはIEEE 754どおり、自分自身とも等しくない

この規則は`contains`のように等価判定を使う組み込み関数でも共有する。

#### 大小比較（`<` / `>` / `<=` / `>=`）

数値（Int / Float）だけを対象とする。IntとFloatは跨いで比較でき、等価比較と同じく数学的に厳密で丸めを経由しない（REV-003）。`2^53`超のIntとFloatの大小も桁落ちせずに決まる。`NaN`が絡む大小比較はすべて`false`、`±Infinity`はすべての有限数と大小が確定する。

数値以外の値、および数値と非数値の組み合わせは`type`エラーになる。文字列の順序比較は提供しない。
...
- `return`には式が必要である。`return`単体はパースエラーになるため、値を返さず抜けたい場合は`return null`と書く
...
import はプログラムの実行を始める前にすべて解決する。読み込み・パース・サンドボックス検査・深度検査は、最初の文を実行する前に行う。

- import が失敗した場合、プログラムは1文も実行されない。失敗より手前に書いた `print` やファイル操作の副作用は起きない
- モジュールのトップレベル文は、`import` 文が書かれていた位置で実行される。したがって正常時の実行順序は、その位置にモジュールの内容を書いた場合と同じになる
- 実行中に生成したファイルを import することはできない。import 対象は実行開始時に存在していなければならない
- 逆に、実行開始後にモジュールファイルを削除しても、解決済みの内容がそのまま実行される
...
| 関数 | 説明 |
|---|---|
| `print(値, ...)` | 値を標準出力に表示。複数引数はスペース区切りで出力。引数0個（`print()`）は空行を出力する |
...
| `keys(dict)` | 辞書のキー一覧をリストで返す（キーのコードポイント順） |
| `type(x)` | 値の型名を文字列で返す |
| `slice(collection, start, end)` | リスト/文字列の部分を切り出す（start から end の手前まで）。`end` は省略できない。負の値は0へクランプするため、インデックスアクセスと違い末尾からの指定にはならない。範囲外は長さへクランプし、`start > end` は空を返す |
...
| `abs(num)` | 絶対値 |
| `min(a, b)` | 小さい方を返す。厳密な数値比較で選び、選ばれた引数を**元の型のまま**返す（`min(2, 2.5)`は`Int`の`2`）。同値なら第1引数、いずれかが`NaN`なら`NaN`を返す |
| `max(a, b)` | 大きい方を返す。厳密な数値比較で選び、選ばれた引数を**元の型のまま**返す（`max(3, 2.5)`は`Int`の`3`）。同値なら第1引数、いずれかが`NaN`なら`NaN`を返す |
...
ユーザー関数呼び出しのネスト深度を128フレームに制限し、通常の関数再帰をプロセスクラッシュ前にランタイムエラー化する。root script frameは数えず、現在activeなユーザー定義の関数・lambda・method・callbackのフレーム数を数える。128個目のユーザーframeは実行でき、129個目を作る直前に拒否する。ツリーウォーク版・VM版で同じ境界・同じメッセージ・同じtraceになる。

- 上限: 128フレーム（root script frameを除くactive user frame数）
- 超過時: 「スタックオーバーフロー: 再帰が深すぎます (上限: 128)」ランタイムエラー
- 対象: 通常呼び出しと`map` / `filter` / `each`のcallback。builtinとcore組み込み関数自体は数えない
- 設定変更: 不可（コンパイル時定数 `MAX_USER_CALL_DEPTH`）
