...
| 1 | 安定した組み込みAPI | [組み込みAPI仕様](embedding-api.md) | ✅ 確定済み | 🟡 tree向け最小facadeとCLI入口に加え、E1（識別子・設定・エラー・terminal outcome 型と `EngineBuilder`、`src/embedding.rs`）を実装。`SourceHash` の SHA-256 計算・`CompiledScript`/`LinkedScript`・最終terminal型・panic境界・CLI統合（E2〜E8a）は未実装 | 第14〜15節 E1〜E6・E8a、EMB-AT-01〜09・13・15〜17 |
...
3. **Phase 1 embedding:** [組み込みAPI仕様](embedding-api.md) ~~E1~~（✅ 完了、`src/embedding.rs`）→E2〜E6→E8a。最終terminal型のsubsetを使い、先行公開型を作らない。
