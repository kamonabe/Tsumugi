...
- [x] セキュリティ強化（コールフレーム深度制限 MAX_CALL_DEPTH=256、map/filter/each ステップカウント修正、TSUMUGI_* 環境変数ブロック）
...
| パースエラーの回復 | 最初の1エラーで停止する代わりに複数エラーをまとめて報告 | 未着手 |
...
| 項目 | 詳細 | 状態 |
|---|---|---|
| `Token::Unknown` のエラーメッセージ改善 | パーサーに流れた際に文字名を含む親切なメッセージにする | 未着手 |
| `!` 単体の処理 | 再帰で不明文字が消える問題を修正 | 未着手 |

### テスト品質

| 項目 | 詳細 | 状態 |
|---|---|---|
| カバレッジ可視化 | `cargo llvm-cov` で未到達パスを特定しテスト拡充 | 未着手 |
| ベンチマーク | `criterion` でリグレッション検出 | 未着手 |
...
| `Env::functions` の廃止 | 関数を変数として統合しスコープルールを一本化 | 未着手 |

### 実行安全性

| 項目 | 詳細 | 状態 |
|---|---|---|
| スタック深度制限 | Rust 実スタックがオーバーフローする前にエラー化 | 未着手 |
