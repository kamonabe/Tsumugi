...
| P0 | REV-015 | source/string/heap/I-O/bulk workが未有限化 | 🟡 | Slice 1（budget型・`BudgetLedger`・legacy adapter）と Slice 2 の string accounting・source/import accounting・heap accounting 基盤（`AllocationLedger`・§5.1 論理サイズ・`HeapBytes` 課金/超過写像。legacy env `TSUMUGI_MAX_LIVE_HEAP_BYTES`）・collection（`List`/`Dict`）の per-drop release（`Rc<Tracked<T>>`＝生成時課金＋最後の参照 drop で release、COW は delta 課金、tree/VM 両対応）と String body の per-drop release（PR-b、`Value::Str` を `Rc<Tracked<String>>` 化、`track_result` で live heap 課金）と cell・tree/VM 関数 instance の per-drop release（PR-c、`SharedValue` を `Rc<Tracked<RefCell<Value>>>` 化、`Value::Fn`/`VmFn` に header token）と AST / bytecode chunk / imported module record / rollback journal の per-drop 追跡（PR-d、`HeapToken = Tracked<()>` トークン。tree は AST・VM は bytecode chunk、import record と journal entry は両 engine）と string リテラル/連結/f-string 経路の課金（生成点で `track_result` を通し cumulative + live heap を課金、tree/VM 両対応）と I-O accounting（input / output / host call の count + bytes を reserve/commit/refund で課金。stdio を host call として co-charge。host call 対象は filesystem read/write と stdio）実装済みで Slice 2 完了。Slice 3 は PR-a（公開 state-machine surface + poll-to-terminal core）・PR-b（tree の statement / block / loop を明示 frame stack + driver ループへ変換し continuation_frame heap を課金/release）・PR-c（関数呼び出し・try handler の明示 frame。`eval_call` / `call_fn_value` を `FrameKind::Call` 化し共通 `run_driver` + `drive_call_body` で駆動。VM の `CallFrame` / `TryHandler` をミラー）を実装済み。全 I/O/host budget の残り（descriptor 上限・stream 途中停止・capability 拒否差）は後続 Slice。Slice 3 は PR-a〜PR-d 完了（PR-d-1 で continuation を Evaluator 所有の `'static` 永続 frame stack へ、PR-d-2 で slice-fuel yield/resume）。Slice 4 は deadline checkpoint（(A)）を実装済み: `BudgetLedger` に `Option<Arc<dyn MonotonicClock>>` を保持し（未注入の既定経路は no-op で観測挙動不変）、`check_deadline` が `clock.now() >= config.deadline` で `ControlStop::DeadlineExceeded` を返す。charge 前（`reserve_all`）と文/反復境界（`run_driver` の `checkpoint_deadline`）で確認し、新 `ErrorKind::DeadlineExceeded` の catch 不能 terminal へ写す（tree の `handle_error`・VM の try handler とも exit と同様に catch 除外、写像は共有 `control_stop_to_error`）。§15.1（deadline 直前は実行・fake clock が deadline 等時点で停止・try/catch で catch body を実行しない）を tree で固定。Slice 4 (B) runtime cancel も実装済み: `CancellationToken::cancel` を `-> bool` 化（最初の false→true が linearization point、§8）、新 `ErrorKind::Cancelled` の catch 不能 terminal（tree `handle_error`・VM try handler とも catch 除外、写像は共有 `control_stop_to_error`）、`BudgetLedger::cancellation_token()` / `Evaluator::cancellation_token()` で token を共有し、charge 前（既存の `check_cancel`）と文/反復境界（`run_driver` の `checkpoint_cancel`）で consult。§8 の cancel（cancel 後は catch 不能 terminal・try/catch で catch body 非実行・cancel() の bool idempotency）を tree で固定。transaction の cancel/deadline rollback 規則（(C)、cancel/deadline とも is_exit=false で rollback 済みだが規則の明文化・pause 跨ぎは残）・pause/resume + ExplicitYield trigger（(D)）・race linearization（(E)）と、公開 embedding API への cancel token / deadline 露出（E11）は Slice 4 / Phase 3 の残り。VM への deadline clock / cancel token 注入・push/pop full-clone 差・f-string リテラル部分の個別課金差は Slice 6。REV表 P0 |
