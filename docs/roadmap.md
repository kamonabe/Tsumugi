...
5. **Phase 2 capability:** E7 → E8b と Capability C1〜C10。**進行中**: C1（`CapabilitySet` 公開surface・deny-by-default・`CapabilitySetId` の byte-level encoding、`src/capability.rs`）を実装済み。次は C2（`CallableCatalog` 統合、AUD-049）→ C3/C4/C5/C7 を並行 → C6（stream 後）→ C8（BuiltinSpec 衝突検査後）→ C9/C10（最後）の順（capability-model §18）。REV-002/009/019/021/022 はこの capability 面で実装（sandbox の process-global を ExecutionContext へ移す）。
