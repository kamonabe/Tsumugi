...
- 128 を選んだ理由: ツリーウォークの再帰は1フレームあたりのRustスタック消費が大きく、Windowsのデフォルトスタック(1MB)でも安全に動作する値
...
- ~~**スタック深度制限**: 深い再帰で Rust の実スタックが溢れるとプロセスごとクラッシュする。ステップ予算では防げない~~ → **解決済み**: `MAX_CALL_DEPTH = 128` でフレーム深度制限を導入
