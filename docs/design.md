...
定数 `MAX_CALL_DEPTH = 64` で関数呼び出しのネスト深度を制限する。超過すると「スタックオーバーフロー: 再帰が深すぎます」ランタイムエラー。

- ツリーウォーク版: `call_stack.len()` でチェック（Rustの実スタックが溢れる前に検出）
- VM版: `frames.len()` でチェック
- 64 を選んだ理由: ツリーウォークの再帰は1フレームあたりのRustスタック消費が大きく、Windowsのデフォルトスタック(1MB)でも安全に動作する値
...
- ~~**スタック深度制限**: 深い再帰で Rust の実スタックが溢れるとプロセスごとクラッシュする。ステップ予算では防げない~~ → **解決済み**: `MAX_CALL_DEPTH = 64` でフレーム深度制限を導入
