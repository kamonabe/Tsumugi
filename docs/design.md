...
定数 `MAX_CALL_DEPTH = 256` で関数呼び出しのネスト深度を制限する。超過すると「スタックオーバーフロー: 再帰が深すぎます」ランタイムエラー。

- ツリーウォーク版: `call_stack.len()` でチェック（Rustの実スタックが溢れる前に検出）
- VM版: `frames.len()` でチェック
- 256 を選んだ理由: ツリーウォークの再帰は1フレームあたりのRustスタック消費が大きく、512ではデフォルトスタック(8MB)が先に溢れるため
...
- ~~**スタック深度制限**: 深い再帰で Rust の実スタックが溢れるとプロセスごとクラッシュする。ステップ予算では防げない~~ → **解決済み**: `MAX_CALL_DEPTH = 256` でフレーム深度制限を導入
